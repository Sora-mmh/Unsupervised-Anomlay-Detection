...
from dataset.stc_dataset import StcDataset_test
from dataset.rails_dataset import RailsDataset_test
...
def test(_class_):
    ...
    test_path = (
        "/home/mmhamdi/workspace/unsupervised/Unsupervised-Anomlay-Detection/data/mvtec/"
        + _class_
    )
    ckp_path = (
        "/home/mmhamdi/workspace/unsupervised/Unsupervised-Anomlay-Detection/sota_models/RD4AD/checkpoints/"
        + "rm_1105_wres50_ff_mm_"
        + _class_
        + ".pth"
    )
    ...
    auroc_px, auroc_sp, aupro_px = evaluation(
        encoder, bn, decoder, test_dataloader, device, _class_
    )
...
def visualization(_class_):
    ...
    if DATASET == "mvtec":
        ...
        pars = get_args()
        data_transform, mask_transform = get_data_transforms(
            pars.image_size, pars.image_size
        )
        test_data = StcDataset_test(
            pars, transform=data_transform, mask_transform=mask_transform
        )

        if DEBUG:
            sampled_test_indices = random.sample(range(len(test_data)), 1)
            test_data = Subset(test_data, sampled_test_indices)
        ...
        test_data = RailsDataset_test(
            pars, transform=data_transform, mask_transform=mask_transform
        )

        if DEBUG:
            sampled_test_indices = random.sample(range(len(test_data)), 32)
            test_data = Subset(test_data, sampled_test_indices)
    ...
    ckp_path = (
        "/home/mmhamdi/workspace/unsupervised/Unsupervised-Anomlay-Detection/sota_models/Revisiting-Reverse-Distillation/results/rails/"
        + _class_
        + "/wres50_"
        + _class_
        + ".pth"
    ...
    ckp = torch.load(
        "/home/mmhamdi/workspace/unsupervised/Unsupervised-Anomlay-Detection/sota_models/Revisiting-Reverse-Distillation/results/rails/rail/wres50_rail.pth"
    )
    for k, v in list(ckp["bn"].items()):
        if "memory" in k:
            ckp["bn"].pop(k)
    ...
    count = 0
    dir_pth = "/home/mmhamdi/workspace/unsupervised/Unsupervised-Anomlay-Detection/sota_models/Revisiting-Reverse-Distillation/"
    with torch.no_grad():
        for img, gt, label, _, _ in test_dataloader:
            ...
            anomaly_map, amap_list = cal_anomaly_map(
                [inputs[-1]], [outputs[-1]], img.shape[-1], amap_mode="a"
            )
            anomaly_map = gaussian_filter(anomaly_map, sigma=4)
            ...
            # if not os.path.exists(dir_pth + "results/" + _class_):
            #     os.makedirs(dir_pth + "results/" + _class_)
            cv2.imwrite(
                dir_pth
                + "results/rails/"
                + _class_
                + "/imgs/"
                + f"original_{str(count).zfill(3)}.png",
                img,
            )
            plt.imshow(img)
            plt.axis("off")
            ...
            cv2.imwrite(
                ...
                + _class_
                + "/imgs/"
                + f"prediction_{str(count).zfill(3)}.png",
                ano_map,
            )
            plt.imshow(ano_map)
            plt.axis("off")
            ...
            cv2.imwrite(
                ...
                + _class_
                + "/imgs/"
                + f"ground_truth_{str(count).zfill(3)}.png",
                gt,
            )
